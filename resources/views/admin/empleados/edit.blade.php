<x-layouts::app title="Editar Empleado">
    <div class="relative mb-6 w-full flex justify-between items-center">
        <div>
            <flux:heading size="xl" level="1">Editar Empleado</flux:heading>
            <flux:subheading>Modifica los datos necesarios del personal seleccionado.</flux:subheading>
        </div>
        <div>
    ...
    <form action="{{ route('admin.empleados.update', $empleado) }}" method="POST">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl p-6 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <flux:input name="nombre" value="{{ old('nombre', $empleado->nombre) }}" placeholder="Ej. Juan Carlos" required />
                        <flux:error name="nombre" />
                    </div>

                    <div>
                        <flux:label>Apellido <span class="text-red-500">(*)</span></flux:label>
                        <flux:input name="apellido" value="{{ old('apellido', $empleado->apellido) }}" placeholder="Ej. Pérez Gómez" required />
                        <flux:error name="apellido" />
                    </div>

                    <div>
                        <flux:label>Cédula de Identidad (CI) <span class="text-red-500">(*)</span></flux:label>
                        <flux:input name="ci" value="{{ old('ci', $empleado->ci) }}" placeholder="Ej. 1234567 SC" required />
                        <flux:error name="ci" />
                    </div>

                    <div>
                        <flux:label>Fecha de Nacimiento</flux:label>
                        <flux:input type="date" name="fecha_nacimiento" value="{{ old('fecha_nacimiento', $empleado->fecha_nacimiento) }}" />
                    ...
                    <div>
                        ...
                        <flux:select placeholder="Seleccionar género..." name="genero">
                            <flux:select.option value="MASCULINO" :selected="old('genero', $empleado->genero) == 'MASCULINO'">MASCULINO</flux:select.option>
                            <flux:select.option value="FEMENINO" :selected="old('genero', $empleado->genero) == 'FEMENINO'">FEMENINO</flux:select.option>
                        </flux:select>
                    ...
                    <div>
                        ...
                        <flux:input name="telefono" value="{{ old('telefono', $empleado->telefono) }}" placeholder="Ej. 70012345" />
                        <flux:error name="telefono" />
                    </div>

                    <div class="md:col-span-2">
                        <flux:label>Dirección domiciliaria</flux:label>
                        <flux:input name="direccion" value="{{ old('direccion', $empleado->direccion) }}" placeholder="Ej. Av. 3 Pasos al Frente #123" />
                        <flux:error name="direccion" />
                    </div>

                    <div>
                        <flux:label>Correo electrónico</flux:label>
                        <flux:input type="email" name="email" value="{{ old('email', $empleado->email) }}" placeholder="correo@ejemplo.com" />
            ...
            <div class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl p-6 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <flux:select placeholder="Selecciona el departamento..." name="departamento_id" required>
                            @foreach ($departamentos as $dep)
                                <flux:select.option value="{{ $dep->id }}" :selected="old('departamento_id', $empleado->departamento_id) == $dep->id">
                                    {{ $dep->nombre }}
                                </flux:select.option>
                            @endforeach
                        </flux:select>
                    ...
                    <div>
                        ...
                        <flux:select placeholder="Selecciona el área..." name="area_id" required>
                            @foreach ($areas as $area)
                                <flux:select.option value="{{ $area->id }}" :selected="old('area_id', $empleado->area_id) == $area->id">
                                    {{ $area->nombre }}
                                </flux:select.option>
                            @endforeach
                        </flux:select>
                    ...
                    <div>
                        ...
                        <flux:input type="date" name="fecha_ingreso" value="{{ old('fecha_ingreso', $empleado->fecha_ingreso) }}" />
                        <flux:error name="fecha_ingreso" />
                    </div>

                    <div>
                        <flux:label>Salario Base <span class="text-red-500">(*)</span></flux:label>
                        <flux:input type="number" step="0.01" name="salario" value="{{ old('salario', $empleado->salario) }}" placeholder="0.00" required />
                    ...
                    <div>
                        ...
                        <flux:select placeholder="Selecciona el estado..." name="estado" required>
                            <flux:select.option value="1" :selected="old('estado', $empleado->estado) == '1'">ACTIVO</flux:select.option>
                            <flux:select.option value="0" :selected="old('estado', $empleado->estado) == '0'">INACTIVO</flux:select.option>
                        </flux:select>
                ...
                <div class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl p-6 shadow-sm">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <flux:input name="banco" value="{{ old('banco', $empleado->banco) }}" placeholder="Ej. Banco Nacional de Bolivia" />
                            <flux:error name="banco" />
                        </div>
                        <div>
                            <flux:label>Número de Cuenta</flux:label>
                            <flux:input name="nro_cuenta" value="{{ old('nro_cuenta', $empleado->nro_cuenta) }}" placeholder="Ej. 4001234567" />
                ...
                <div class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl p-6 shadow-sm">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <flux:input name="celular_referencia" value="{{ old('celular_referencia', $empleado->celular_referencia) }}" placeholder="Ej. 79876543" />
                            <flux:error name="celular_referencia" />
                        </div>
                        <div>
                            <flux:label>Parentesco de la Referencia</flux:label>
                            <flux:input name="parentesco_referencia" value="{{ old('parentesco_referencia', $empleado->parentesco_referencia) }}" placeholder="Ej. Esposo/a, Padre, Hermano/a" />
            ...
            <div class="flex justify-end gap-3 pt-2">
                ...
                <flux:button variant="primary" type="submit" color="blue" icon="check">Actualizar Empleado</flux:button>
            </div>
