<x-layouts::app title="Gestión de Finiquitos">
    @php
        $ajusteDivisa = \App\Models\Ajuste::first()->divisas ?? 'BOB';
        $jsonPath = public_path('divisas.json');
        $simboloMoneda = $ajusteDivisa;
        if (file_exists($jsonPath)) {
            $divisasData = json_decode(file_get_contents($jsonPath), true);
            if (isset($divisasData[$ajusteDivisa]['symbol'])) {
                $simboloMoneda = $divisasData[$ajusteDivisa]['symbol'];
            }
        }
    @endphp

    <div class="relative mb-6 w-full flex justify-between items-center">
        <div>
            <flux:heading size="xl" level="1">Finiquitos y Liquidaciones</flux:heading>
            <flux:subheading>Historial de desvinculaciones y cálculo de beneficios sociales.</flux:subheading>
        </div>
        <!-- Botón para redireccionar a la vista de creación protegido -->
        @can('admin.finiquitos.create')
            <div>
                <a href="{{ route('admin.finiquitos.create') }}">
                    <flux:button variant="primary" icon="plus" color="blue">Nuevo Finiquito</flux:button>
                </a>
            </div>
        @endcan
    </div>

    <!-- Buscador -->
    <div class="flex gap-4 mb-6">
        <div class="flex-1">
            <form action="{{ route('admin.finiquitos.index') }}" method="GET" class="flex gap-2 w-1/2">
                <div class="flex-1">
                    <flux:input name="buscar" type="text" icon="magnifying-glass" placeholder="Buscar por empleado o causal de retiro..."
                        value="{{ $buscar ?? '' }}" />
                </div>
                <button type="submit" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg transition flex items-center gap-2 cursor-pointer">
                    <i class="fas fa-search"></i> Buscar
                </button>
                @if (isset($buscar) && $buscar != '')
                    <a href="{{ route('admin.finiquitos.index') }}" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-lg transition flex items-center gap-2">
                        <i class="fas fa-trash"></i> Limpiar
                    </a>
                @endif
            </form>
        </div>
    </div>

    <flux:separator variant="subtle" class="mb-6" />

    <!-- Tabla -->
    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-sm">
        <table class="min-w-full border-collapse">
            <thead class="bg-gray-50 dark:bg-zinc-900 text-center">
                <tr>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Nro</th>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Empleado</th>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Ingreso</th>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Retiro</th>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Causal</th>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Años Serv.</th>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Total a Pagar</th>
                    <th class="px-4 py-3 border-x border-b border-gray-200 dark:border-zinc-700 text-xs font-bold text-gray-500 uppercase">Acciones</th>
                </tr>
            </thead>
            <tbody class="bg-white dark:bg-zinc-800 text-center">
                @forelse ($finiquitos as $finiquito)
                    <tr class="hover:bg-blue-50 dark:hover:bg-zinc-700/50 transition">
                        <td class="px-3 py-3 border border-gray-200 dark:border-zinc-700 text-sm">{{ ($finiquitos->currentPage() - 1) * $finiquitos->perPage() + $loop->iteration }}</td>
                        <td class="px-4 py-3 border border-gray-200 dark:border-zinc-700 text-sm text-left font-semibold">
                            {{ $finiquito->empleado->nombre ?? 'N/A' }} {{ $finiquito->empleado->apellido ?? '' }}
                            <div class="text-xs text-gray-500 font-normal">CI: {{ $finiquito->empleado->ci ?? 'N/A' }}</div>
                        </td>
                        <td class="px-4 py-3 border border-gray-200 dark:border-zinc-700 text-sm">
                            {{ \Carbon\Carbon::parse($finiquito->fecha_ingreso)->format('d/m/Y') }}
                        </td>
                        <td class="px-4 py-3 border border-gray-200 dark:border-zinc-700 text-sm">
                            {{ \Carbon\Carbon::parse($finiquito->fecha_retiro)->format('d/m/Y') }}
                        </td>
                        <td class="px-4 py-3 border border-gray-200 dark:border-zinc-700 text-sm">
                            <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">
                                {{ $finiquito->causal_retiro }}
                            </span>
                        </td>
                        <td class="px-4 py-3 border border-gray-200 dark:border-zinc-700 text-sm font-medium">
                            {{ $finiquito->anos_servicio }} años
                        </td>
                        <td class="px-4 py-3 border border-gray-200 dark:border-zinc-700 text-sm font-bold text-emerald-600 dark:text-emerald-400">
                            {{ $simboloMoneda }} {{ number_format($finiquito->total_beneficios, 2, ',', '.') }}
                        </td>
                        <td class="px-4 py-3 border border-gray-200 dark:border-zinc-700">
                            <div class="flex justify-center items-center gap-1.5">

                                <!-- Ver protegido -->
                                @can('admin.finiquitos.show')
                                    <a href="{{ route('admin.finiquitos.show', $finiquito->id) }}" class="px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium rounded-md shadow-sm transition flex items-center">
                                        <i class="fas fa-eye mr-1"></i> Ver
                                    </a>
                                @endcan

                                <!-- Imprimir protegido -->
                                @can('admin.finiquitos.print')
                                    <a href="{{ route('admin.finiquitos.print', $finiquito->id) }}" target="_blank" class="px-3 py-1.5 bg-zinc-600 hover:bg-zinc-700 text-white text-xs font-medium rounded-md shadow-sm transition flex items-center">
                                        <i class="fas fa-print mr-1"></i> Imprimir
                                    </a>
                                @endcan

                                <!-- Eliminar protegido -->
                                @can('admin.finiquitos.destroy')
                                    <form action="{{ route('admin.finiquitos.destroy', $finiquito->id) }}" method="POST" class="inline-flex" id="formEliminarFiniquito{{ $finiquito->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs font-medium rounded-md shadow-sm transition cursor-pointer flex items-center" onclick="confirmarEliminacionFiniquito{{ $finiquito->id }}(event)">
                                            <i class="fas fa-trash-alt mr-1"></i> Eliminar
                                        </button>
                                    </form>
                                    <script>
                                        function confirmarEliminacionFiniquito{{ $finiquito->id }}(e) {
                                            e.preventDefault();
                                            Swal.fire({
                                                title: '¿Eliminar registro de finiquito?',
                                                text: "¡No podrás revertir esta acción!",
                                                icon: 'warning',
                                                showCancelButton: true,
                                                confirmButtonColor: '#d33',
                                                cancelButtonColor: '#3085d6',
                                                confirmButtonText: 'Sí, eliminar',
                                                cancelButtonText: 'Cancelar'
                                            }).then((result) => {
                                                if (result.isConfirmed) {
                                                    document.getElementById('formEliminarFiniquito{{ $finiquito->id }}').submit();
                                                }
                                            });
                                        }
                                    </script>
                                @endcan
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="px-6 py-6 text-center text-sm text-gray-500 dark:text-zinc-400">No hay registros de finiquitos o liquidaciones.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    </div>

    @if ($finiquitos->hasPages())
        <div class="mt-4">{{ $finiquitos->appends(['buscar' => $buscar])->links() }}</div>
    @endif
</x-layouts::app>